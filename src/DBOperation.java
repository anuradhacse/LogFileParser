...
public class DBOperation {
    ...
    public int getFileNameValue(String path) throws SQLException{
        setConenction();
        pst = con.prepareStatement("SELECT file_id FROM 356021071492850_files WHERE path = ?");
    ...
    public int getFileSize(String path) throws SQLException{
        setConenction();
        pst = con.prepareStatement("SELECT size FROM 356021071492850_files WHERE path = ?");
        pst.setString(1,path);
        resultSet=pst.executeQuery();
        int fileSize=0;
        if(resultSet.next()){
